...
struct Biblioteca initializare(int cod, const char* nume, unsigned int nrCarti, const float* preturi, float suprafata) {
	...
	b.nume = calloc((strlen(nume) + 1), sizeof(char));
	if (b.nume != NULL) {
		strcpy(b.nume, nume);
	}

	b.preturi = calloc(nrCarti, sizeof(float));
	if (b.preturi != NULL) {
...
struct Biblioteca copiereBiblioteca(struct Biblioteca b) {
	...
	copie.nume = calloc((strlen(b.nume) + 1), sizeof(char));
	if (copie.nume != NULL) {
		strcpy(copie.nume, b.nume);
	}

	copie.preturi = calloc(b.nrCarti, sizeof(float));
...
void push_biblioteca(struct Biblioteca b, struct Biblioteca** biblioteci, int* nrBiblioteci) {
	(*nrBiblioteci)++;
	struct Biblioteca* nou = calloc((*nrBiblioteci), sizeof(struct Biblioteca));
